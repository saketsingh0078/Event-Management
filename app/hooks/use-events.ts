...
async function fetchEvents(filters?: EventFilters): Promise<EventsResponse> {
  ...
  if (!contentType || !contentType.includes('application/json')) {
    ...
    throw new Error(
      `Server returned non-JSON response. This usually means the database is not configured. Please check your DATABASE_URL environment variable. Response: ${text.substring(0, 200)}`,
    )
  }
...
async function updateEvent(id: number, data: UpdateEventInput): Promise<Event> {
  const response = await fetch(`/api/events/${id}`, {
    method: 'PUT',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify(data),
